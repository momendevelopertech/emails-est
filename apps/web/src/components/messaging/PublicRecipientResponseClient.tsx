...
export default function PublicRecipientResponseClient({ dashboardHref }: { dashboardHref?: string }) {
            ...
            <div className="mx-auto max-w-4xl rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5 md:p-8">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <h1 className="mt-4 text-3xl font-semibold text-slate-950">Confirm or apologize for this assignment</h1>
                        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                            Review the assignment summary below, then choose the response that reflects the proctor's availability.
                        </p>
                    </div>
                ...
                <div className="mt-8 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                        <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4">
                            ...
                            <div className="mt-2 text-sm font-semibold text-slate-900">{recipient?.room_est1 || '-'}</div>
                        </div>
                        <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4">
                            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Role</div>
                            <div className="mt-2 text-sm font-semibold text-slate-900">{recipient?.role || '-'}</div>
                        </div>
                        <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4">
                            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Type</div>
                            <div className="mt-2 text-sm font-semibold text-slate-900">{recipient?.type || '-'}</div>
                        </div>
                        <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4">
                            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Governorate</div>
                            <div className="mt-2 text-sm font-semibold text-slate-900">{recipient?.governorate || '-'}</div>
                        </div>
                    </div>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    ...
                    {dashboardHref ? (
                        <Link href={dashboardHref} className="btn-outline min-h-12">
                            Return to dashboard
                        </Link>
                    ) : null}
                </div>
