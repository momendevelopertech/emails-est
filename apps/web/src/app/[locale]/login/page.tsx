...
export default function LoginPage({ params }: { params: { locale: 'en' | 'ar' } }) {
    ...
    const availableDepartments = useMemo(() => {
        ...
        return departments.filter((department) =>
            department.branches?.some((branch) => String(branch.id) === registerForm.branchId),
        );
    ...
    const submitLogin = async () => {
        if (loading || checkingSession || !validateLogin()) return;
        setLoading(true);
        setErrors({});
        try {
            await api.get('/auth/csrf');
    ...
    const submitRegister = async () => {
        if (loading || checkingSession || !validateRegister()) return;
        setLoading(true);
        setErrors({});
        try {
            await api.get('/auth/csrf');
                ...
                {mode === 'login' ? (
                    ...
                    >
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <label className="text-sm">
                                ...
                                <input
                                    type="text"
                                    className="mt-1 w-full rounded-xl border border-ink/20 bg-white px-3 py-2"
                                    value={registerForm.fullNameAr}
                                    onChange={(e) => updateRegisterField('fullNameAr', e.target.value)}
                                />
                            </label>
                            <label className="text-sm">
                                {t('registerEmail')}
                            ...
                            <label className="text-sm">
                                ...
                                <input
                                    ...
                                    onChange={(e) => updateRegisterField('jobTitle', e.target.value)}
                                />
                            ...
                            <label className="text-sm">
                                ...
                                <input
                                    type="text"
                                    className="mt-1 w-full rounded-xl border border-ink/20 bg-white px-3 py-2"
                                    value={registerForm.jobTitleAr}
                                    onChange={(e) => updateRegisterField('jobTitleAr', e.target.value)}
                                />
                            </label>
                            <label className="text-sm">
                                {t('registerBranch')}
                                <select
                                    ...
                                    onChange={(e) => updateRegisterField('branchId', e.target.value)}
                                    disabled={loadingRegisterOptions}
                                >
                            ...
                            <label className="text-sm">
                                ...
                                <select
                                    ...
                                    onChange={(e) => updateRegisterField('departmentId', e.target.value)}
                                    disabled={!registerForm.branchId || loadingRegisterOptions}
                                >
                        ...
                        <button
                            className="btn-primary w-full"
                            type="submit"
                            disabled={loading || checkingSession || loadingRegisterOptions}
                        >
                            {loading
                                ? (params.locale === 'ar' ? 'جارٍ إنشاء الحساب...' : 'Creating account...')
                                : t('register')}
                        </button>
